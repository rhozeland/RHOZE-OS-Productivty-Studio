...
const AnimatedTutorialDemo = ({ onBack, onContinue }: { onBack: () => void; onContinue: () => void }) => {
  ...
  useEffect(() => {
    const runCycle = () => {
      setAnimating(true);
      // Card flies out, then resets after a pause
  ...
  return (
    ...
    >
      ...
      >
        {/* Direction labels around the card */}
        <motion.div
        ...
        <motion.div
          className="absolute top-1/2 -translate-y-1/2 flex items-center gap-0.5 z-10"
          style={{ left: -52 }}
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {/* Action label overlay when animating */}
            <AnimatePresence>
              {animating && (
                <motion.div
                ...
                >
                  <span className={`font-display text-lg font-bold tracking-wider ${current.color}`}>
                    {current.label}
                  </span>
                </motion.div>
              )}
        ...
        <AnimatePresence>
          {animating && (
            ...
            >
              <motion.div
                className="h-8 w-8 rounded-full bg-primary/15"
                animate={{
                  x: current.x * 0.3,
                  y: current.y * 0.3,
                  scale: [1, 1.5, 0],
      ...
      >
        <Button variant="outline" onClick={onBack} className="rounded-full px-7 h-10 text-sm">
          Back
        </Button>
        <Button onClick={onContinue} className="rounded-full px-8 h-10 text-sm shadow-md">
          Try it yourself
        </Button>
      </motion.div>
    </motion.div>
