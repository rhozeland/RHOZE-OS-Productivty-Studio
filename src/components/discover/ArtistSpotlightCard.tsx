  ...
  return (
    ...
    >
      ...
      <div className="px-5 pt-6 pb-5">
        ...
        {/* Visual mini-tile for next event — clickable through to detail.
            v11 — "Hosts <space>" tile removed per product call: featured
            creator cards lead with the artist + their next happening, not
            their studio/space ownership. */}
        {eventHref && next_event && (
          <div className="mt-5">
            <Link
              to={eventHref}
              onClick={stop}
              className="group/tile relative flex h-20 w-full overflow-hidden rounded-xl border border-border/55 bg-muted/40 transition-all hover:border-foreground/45 hover:shadow-[0_10px_28px_hsl(var(--background)/0.45)]"
      ...
      <div className="border-t border-border/40 bg-background/40 px-5 py-4">
        ...
        >
          {isSelf ? "View your profile" : `Support ${firstName(title)}`}
