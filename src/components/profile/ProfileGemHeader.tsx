/**
 * ProfileGemHeader — unified "gem card" header that merges the old
 * banner + identity block + InvestorSignalStrip + CreatorActivityTicker
 * into a single integrated surface. Modeled after the Creator Pass card.
 *
 * Shows:
 *  - Banner with Boost/Edit (owner) or Follow/Message (visitor)
 *  - Avatar · name · @handle · location · archetype · tier
 *  - Bio + portfolio + social icons
 *  - Primary CTA: "Support {name}" for visitors (opens SupportSheet)
 *  - Integrated stats strip: Readiness · Verified IP · Contributions ·
 *    Active 30d · Tenure · most-recent activity timestamp
 *
 * Replaces the previous fragmented stack of header + strip + ticker.
 */
...
import {
  Settings, Sparkles, UserPlus, UserCheck, MessageSquare, Heart,
  MapPin, Star, Globe, ExternalLink, Shield, ShieldCheck, Activity, Clock,
  BadgeCheck, Loader2,
} from "lucide-react";
...
export const ProfileGemHeader = ({
  ...
  const bannerGradient = p.banner_gradient || archetypeBannerGradient(p.archetype, id);
  ...
  const { data: signals, isLoading: signalsLoading } = useQuery<Signals>({
    ...
    queryFn: async () => {
      ...
      const [worksRes, verifiedRes, eventsRes, contribRes, flowRes, latestRes] = await Promise.all([
        ...
        supabase.from("flow_items").select("created_at, category").eq("user_id", id).order("created_at", { ascending: false }).limit(1).maybeSingle(),
      ]);
      return {
        ...
        latestActivityAt: (latestRes.data as any)?.created_at ?? null,
        latestActivityKind: (latestRes.data as any)?.category ?? null,
      };
    },
    staleTime: 60_000,
  });
  ...
  const tier =
    score >= 70
      ? { label: "Strong signal", barClass: "bg-emerald-500", textClass: "text-emerald-600 dark:text-emerald-400" }
      : score >= 35
        ? { label: "Building signal", barClass: "bg-amber-500", textClass: "text-amber-600 dark:text-amber-400" }
        : { label: "Early signal", barClass: "bg-muted-foreground/60", textClass: "text-muted-foreground" };

  const stats = signals ? [
    { Icon: ShieldCheck, label: "Verified IP", value: `${signals.verifiedWorks}/${Math.max(signals.totalWorks, 1)}` },
    { Icon: Sparkles, label: "Contributions", value: String(signals.contributions) },
    { Icon: Activity, label: "Active 30d", value: String(signals.flowPosts30d) },
  ...
  return (
    <motion.div
      ...
      className="rounded-3xl bg-card/80 backdrop-blur-sm border border-border/50 shadow-lg overflow-hidden"
    >
      {/* Banner */}
      <div className="h-36 sm:h-44 relative" style={{ background: bannerGradient }}>
        ...
        {isOwnProfile ? (
          <div className="absolute top-3 right-3 flex items-center gap-2">
            <Button
              variant="secondary" size="sm" onClick={onBoost}
            ...
            <Button
              variant="secondary" size="sm" onClick={onEditProfile}
          ...
          <div className="absolute top-3 right-3 flex items-center gap-2">
            <Button
              size="sm" onClick={onFollow} disabled={followPending}
              className="gap-1.5 bg-card/80 backdrop-blur-sm hover:bg-card/95 text-foreground shadow-md text-xs"
              variant="secondary"
            >
              {isFollowing ? <><UserCheck className="h-3.5 w-3.5" /> Following</> : <><UserPlus className="h-3.5 w-3.5" /> Follow</>}
            </Button>
            <Button
              variant="secondary" size="sm" onClick={onMessage}
              className="gap-1.5 bg-card/80 backdrop-blur-sm hover:bg-card/95 shadow-md text-xs"
            >
      ...
      <div className="px-5 sm:px-8 pb-6 pt-3">
        ...
        <div className="flex items-end gap-4 sm:gap-5">
          ...
          <div className="flex-1 min-w-0 pb-1">
                ...
                <HoverCard openDelay={120} closeDelay={80}>
                  ...
                  <HoverCardContent align="start" className="w-64 text-xs">
                    ...
                    <p className="mt-1 text-muted-foreground">
                      Identity confirmed by Rhozeland.
                    </p>
                  </HoverCardContent>
                </HoverCard>
              )}
            ...
            {p.username && (
              <p className="text-sm text-muted-foreground leading-none mt-1.5">@{p.username}</p>
            )}
            {(locationLabel || p.archetype) && (
        ...
        {!isOwnProfile && authedUser && (
          <div className="mt-5 flex flex-wrap items-center gap-2">
            <Button
              ...
              className="gap-1.5 bg-gradient-to-r from-primary to-fuchsia-500 hover:opacity-90 text-primary-foreground shadow-lg"
            >
              <Heart className="h-4 w-4" />
              Support {p.display_name || p.username || "creator"}
            </Button>
            <Button variant="outline" size="lg" onClick={() => navigate(`/projects/new?with=${id}`)} className="gap-1.5">
              <Sparkles className="h-4 w-4" /> Start a project
            </Button>
            <Button variant="ghost" size="sm" onClick={onShareCard}>
              Share card
            </Button>
          </div>
        )}
            ...
            <>
              <div className="flex items-center gap-4 sm:gap-5">
                <div className="text-right shrink-0">
                  <p className="font-mono text-2xl sm:text-3xl font-bold leading-none tabular-nums text-foreground">{score}</p>
                  <p className="text-[9px] uppercase tracking-[0.18em] text-muted-foreground mt-1">Readiness</p>
                </div>
                <div className="hidden sm:block w-px h-12 bg-border/60" />
                <div className="flex-1 min-w-0">
                  <p className={cn("text-xs font-semibold leading-none", tier.textClass)}>
                    <span className="font-display">{tier.label}</span>
                    <span className="text-muted-foreground font-normal ml-2">· Investor signal</span>
                  </p>
                  <div className="mt-2 h-1.5 rounded-full bg-muted/50 overflow-hidden">
                    <div className={cn("h-full transition-all", tier.barClass)} style={{ width: `${score}%` }} />
                  </div>
                  {signals.latestActivityAt && (
                    <p className="mt-2 text-[11px] text-muted-foreground flex items-center gap-1.5">
                      <Activity className="h-3 w-3" />
                      Last activity {formatDistanceToNowStrict(new Date(signals.latestActivityAt), { addSuffix: true })}
                    </p>
                  )}
                </div>
                {isOwnProfile && (
              ...
              <div className="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-2">
                {stats.map((s) => (
                  <div
                    key={s.label}
                    className="rounded-xl bg-card/60 border border-border/40 px-3 py-2.5"
                  >
                    <div className="flex items-center gap-1.5 text-muted-foreground">
                      <s.Icon className="h-3 w-3" />
                      <span className="text-[9px] uppercase tracking-[0.18em] font-semibold">{s.label}</span>
                    </div>
                    <p className="font-display text-lg font-bold tabular-nums leading-none mt-1.5 text-foreground">
                      {s.value}
                    </p>
                  </div>
                ))}
